...
public class Main {
  ...
  static void bfs(int y, int x) {
    ...
    q.offer(new Node(0,0,1,0)); vis[0][0][0]=true;
    while (!q.isEmpty()) {
      Node cur=q.poll();
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (nxtY>=0 && nxtY<n && nxtX>=0 && nxtX<m) {
          ...
          if (graph[nxtY].charAt(nxtX)=='0') {
            if (cur.destroyed<=k && !vis[nxtY][nxtX][cur.destroyed]) {
              q.offer(new Node(nxtY,nxtX,cur.cnt+1,cur.destroyed));
              vis[nxtY][nxtX][cur.destroyed]=true;
            }
          }

          else {
            if (cur.destroyed<k && !vis[nxtY][nxtX][cur.destroyed+1]) {
              //graph 바꾸지 말기(벽도 0처럼)
              q.offer(new Node(nxtY,nxtX,cur.cnt+1,cur.destroyed+1));
              vis[nxtY][nxtX][cur.destroyed+1]=true;
            }
          }
        }
      }
